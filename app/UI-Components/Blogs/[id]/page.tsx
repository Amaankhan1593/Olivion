    ...
    return (
        <div className="section-bg text-white flex flex-col">
             ...
             <div className="flex items-center gap-5 mt-3">
                <Link href="/UI-Components/Blogs" className="hover:text-(--prim) text-xl transition-all
                duration-300 mt-5 border border-gray-300 px-4 py-2 rounded 
                hover:bg-white">Back To Blogs</Link>
             </div>
        </div>
    )
   }
    ...
    <>
      <div className="section-bg text-white flex flex-col">
        ...
        <div className="flex items-center text-xl mt-3">
          ...
          <i className="ri-arrow-right-wide-fill mt-1"></i>
          <h2 className="GolosText text-white text-2xl md:text-lg mt-5 md:mt-0">{blog.title}</h2>
        </div>
      </div>
        ...
        <div className="w-full flex flex-col lg:flex-row justify-between gap-8">
          {/* Blog content */}
           <div className="w-full lg:w-1/1">
             ...
             <p className="text-gray-500 GolosText mt-5 leading-relaxed">Modern interior design prioritizes furniture that is both aesthetically pleasing and highly functional. Look for multi-purpose furniture such as extendable dining tables, sofa beds, or modular shelving units that provide storage solutions while maintaining a sleek look. Additionally, investing in ergonomic seating and workspaces enhances comfort and practicality, especially for home offices.</p>
           
             <div className="my-5">
               <div className="bg-[#efebe8] rounded-2xl flex flex-col justify-center 
               items-center text-center px-10 py-8">
                  <Image
                   src={quote}
                   alt="quote"
                   className="rounded-2xl "
                  />
                  <p className="GolosText text-2xl mb-3">“Modern interior design transforms ordinary spaces extraordinary environments. The core principle of modern design is “less is more,” focusing on simplicity and the elimination of clutter.”</p>
                  <h4 className="GolosText font-bold">Aaliyah Brown</h4>
               </div>
             </div>
             <h2 className="text-4xl md:text-4xl CalSans my-5">Bringing Modern Interior Design</h2>
             <p className="text-gray-500 GolosText mt-5 leading-relaxed">Incorporating these modern interior design tips, you can transform your home into a stylish, functional, and comfortable haven. Whether you’re making small updates or going for a full renovation, focusing on minimalism, smart technology, natural elements, and functional and enhance the modern look.</p>
           </div>
           ...
           <div className="w-full lg:w-1/2 sticky top-22 left-0 h-full">
             <div>
              ...
              <div className="flex flex-col gap-2">
                <h2 className="GolosText text-lg hover:text-(--prim) cursor-pointer
                transition-all duration-300 font-semibold border-b border-gray-300
                pb-1">Accessories</h2>
                <h2 className="GolosText text-lg hover:text-(--prim) cursor-pointer
                transition-all duration-300 font-semibold border-b border-gray-300
                pb-1">Electrical & Lighting</h2>
                <h2 className="GolosText text-lg hover:text-(--prim) cursor-pointer
                transition-all duration-300 font-semibold border-b border-gray-300
                pb-1">Home Appliance</h2>
                <h2 className="GolosText text-lg hover:text-(--prim) cursor-pointer
                transition-all duration-300 font-semibold border-b border-gray-300
                pb-1">Power Tools</h2>
                <h2 className="GolosText text-lg hover:text-(--prim) cursor-pointer
                transition-all duration-300 font-semibold border-b border-gray-300
                pb-1">Uncategorized</h2>
                <h2 className="GolosText text-lg hover:text-(--prim) cursor-pointer
                transition-all duration-300 font-semibold border-b border-gray-300
                pb-1">Ware Accessories</h2>
              </div>
             </div>
              <div>
                    ...
                    <Link href={`/UI-Components/Blogs/${blog.id}`}>
                       <div className="flex flex-row gap-3">
                         ...
                         <div className="flex flex-col">
                           ...
                           <h2 className="GolosText text-xl md:text-3xl lg:text-xl
                           hover:text-(--prim) cursor-pointer font-semibold
                           transition-all duration-300">{blog.title}</h2>
                         </div>
                       </div>
                    </Link>
                  ))}
